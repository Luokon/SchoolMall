<script setup name="user">
  ...
  const rules = reactive({
    ...
    reply:[{required: true, message: '必选项不能为空', trigger: 'blur'}],
  ...
  const handleUpdate = async (id) => {
    ...
    const res = await request.get(`/message/${id}`)
  ...
  const handleBatchDel = async () => {
    ...
    await request.delete(`/message/batch/${ids}`)
    ElMessage({
      showClose: true,
      message: '批量删除成功',
      type: 'success'
  ...
  const load = async () => {
    const res = await request.get('/message/page', {
      params: {
        name: name.value,
    ...
    await formEl.validate(async (valid, fields) => {
      if (valid) {
        await request({
          ...
          url: form.id ? `/message/${form.id}` : '/message',
          data: form
        })
        ElMessage({
          showClose: true,
  ...
  const beforeBatchUpload = async (file) => {
    let fd = new FormData()
    fd.append('file', file)
    await request.post('/message/batch/upload', fd, {
      headers: {
...
<template>
  ...
  <el-row>
    ...
    <el-col style="margin-bottom: 10px; display: flex; gap: 10px">
      <!--   新增   -->
      <div v-permission="'sys:message:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:message:batch:upload'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:message:batch:delete'">
        <el-popconfirm
      ...
      <div v-permission="'sys:message:batch:export'">
        <el-button
                type="warning"
                :icon="Download"
                :disabled="disabled"
    ...
    <el-col>
      ...
      >
        ...
        <el-table-column prop="des" label="详情"  align="center"/>
        ...
        <el-table-column prop="statu" label="当前状态"  align="center">
          <template #default="scope">
            <el-tag v-if="scope.row.statu===0">未回复</el-tag>
            <el-tag v-if="scope.row.statu===1" type="success">已回复</el-tag>
          </template>
        </el-table-column>
            ...
            <div style="display: flex; justify-content: center; gap: 10px">
              <div>
                <el-button
                        ...
                        v-permission="'sys:message:update'"
                        v-if="scope.row.statu===0"
                >回复
                </el-button>
              </div>
              <div v-permission="'sys:message:delete'">
                <el-popconfirm
                        confirm-button-text="确定"
                        cancel-button-text="取消"
                        :icon="InfoFilled"
