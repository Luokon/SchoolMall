<script setup >
  ...
  const rules = reactive({
    ...
    time:[{required: true, message: '必选项不能为空', trigger: 'blur'}],
  })
  ...
  const handleBatchDel = async () => {
    ...
    await request.delete(`/cart/batch/${ids}`)
    ElMessage({
      showClose: true,
      message: '批量删除成功',
      type: 'success'
  ...
  const load = async () => {
    const res = await request.get('/cart/page', {
      params: {
        name: name.value,
  ...
  const beforeBatchUpload = async (file) => {
    let fd = new FormData()
    fd.append('file', file)
    await request.post('/cart/batch/upload', fd, {
      headers: {
...
<template>
  ...
  >
    ...
    >
        ...
        <el-form-item label="创建账户" prop="name">
  ...
  <el-row>
    ...
    <el-col style="margin-bottom: 10px; display: flex; gap: 10px">
      <!--   新增   -->
      <div v-permission="'sys:cart:add'">
        <el-button type="primary" :icon="Plus" @click="handleAdd" size="small" plain
        >新增
        </el-button>
      </div>
      <!--   批量导入   -->
      <div v-permission="'sys:cart:batch:upload'">
      ...
      <!--   批量删除   -->
      <div v-permission="'sys:cart:batch:delete'">
        <el-popconfirm
      ...
      <div v-permission="'sys:cart:batch:export'">
        <el-button
                type="warning"
                :icon="Download"
                :disabled="disabled"
    ...
    <el-col>
      ...
      >
        ...
        <el-table-column prop="name" label="创建账户"  align="center"/>
        <el-table-column prop="gid" label="商品编号"  align="center"/>
            ...
            <div style="display: flex; justify-content: center; gap: 10px">
              <div>
                <el-button
                        ...
                        v-permission="'sys:cart:update'"
                >编辑
                </el-button>
              </div>
              <div v-permission="'sys:cart:delete'">
                <el-popconfirm
                        confirm-button-text="确定"
                        cancel-button-text="取消"
                        :icon="InfoFilled"
